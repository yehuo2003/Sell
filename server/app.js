...
//2.2：配置允许脚手架访问程序
app.use(cors({
  origin:["http://127.0.0.1:3000","http://localhost:8080","http://127.0.0.1:8080"],
  credentials:true
}))
...
app.use(session({
  ...
  resave:false,          //请求保存
  seveUninitialized:true,//初始化保存
  cookie:{
    maxAge: 1000 * 60 * 60  //存活时间
  }
}));

app.listen(3000,()=>{
  console.log("服务器启动成功，端口3000，ID地址：176.137.6.75");
});
